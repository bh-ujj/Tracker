...
import com.ujwal.locate2.view.faculties.FacultyLogin;
import com.ujwal.locate2.view.students.StudentLogin;
...
public class ForgotPassword extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Glide.with(ForgotPassword.this).load(R.drawable.realtime).into(imageView);
                        ...
                        public void onComplete(@NonNull Task<Void> task) {
                                if(task.isSuccessful())
                                {
                                    new MaterialDialog.Builder(ForgotPassword.this).title("Password reset link sent.\nIf you have recieved it, then tap on \'Exit\' & change your password via link.\nDidn't recieve it? Try Again!")
                                            .positiveText("Exit").negativeText("Try Again!").canceledOnTouchOutside(false).cancelable(false).positiveColor(getColor(R.color.colorPrimaryDark))
                                            ...
                                            }).negativeColor(getColor(R.color.colorPrimaryDark)).onNegative(new MaterialDialog.SingleButtonCallback() {
                                        @Override
                                        public void onClick(@NonNull MaterialDialog dialog, @NonNull DialogAction which) {
                                            email.setText("");
                                            dialog.cancel();
